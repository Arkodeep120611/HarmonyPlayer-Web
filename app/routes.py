...
def _owned_playlists():
    return current_user.playlists.order_by(
        Playlist.created_at.desc()
    ).all()


def _owned_songs():
    return current_user.songs.order_by(
...
def upload():
    ...
    if not form.validate_on_submit():
        flash(
            "Upload failed. Check the file and metadata fields.",
            "error",
        )
        return redirect(url_for("main.library"))
    ...
    if not original_name or not allowed_audio_file(original_name):
        flash(
            "Only MP3, WAV, OGG, FLAC, and M4A files are allowed.",
            "error",
        )
        return redirect(url_for("main.library"))
    ...
    try:
        # Temporarily save the file locally so Mutagen can inspect it.
        upload_file.save(absolute_path)

        fallback_title = (
            form.title.data.strip()
            if form.title.data
            else Path(original_name).stem
        )
        fallback_artist = (
            form.artist.data.strip()
            if form.artist.data
            else "Unknown Artist"
        )
        fallback_album = (
            form.album.data.strip()
            if form.album.data
            else "Unknown Album"
        )
        ...
        # Upload to persistent Backblaze B2 storage.
        upload_file_to_b2(
            absolute_path,
            relative_path,
        ...
        # If the B2 upload succeeded but the database operation failed,
        # remove the orphaned B2 object.
        try:
        ...
        # The Render filesystem is only used as temporary storage.
        try:
...
def stream_song(song_id: int):
    ...
    try:
        # Generate a temporary private B2 URL.
